...
p.prior <- prior.p.dmc(
  ...
  p1 = p.vector,                           
  p2 = c(0.2, 0.2, rep(1, 10), rep(1.5, 10)), 
  lower = c(0.1, 0, rep(0, 10), rep(-Inf, 10)),
  upper = c(1, 10, rep(Inf, 10), rep(Inf, 10))
)
length(p.prior)
length(p.vector)
...
save(dm, file = "samples/dmAutoPM_B.RData")
...
# Save
save(samples, file = "samples/sAutoPM_B.RData")
...
# 
# # Save
# save(hsamples, file = "samples/hsAutoPM_B.RData")