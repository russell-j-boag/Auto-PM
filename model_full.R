...
p.prior <- prior.p.dmc(
  ...
  p1 = p.vector,                           
  p2 = c(0.2, 0.2, rep(1, 10), rep(1.5, 40)), 
  lower = c(0.1, 0, rep(0, 10), rep(-Inf, 40)),
  upper = c(1, 10, rep(Inf, 10), rep(Inf, 40))
)
length(p.prior)
length(p.vector)
...
save(dm, file = "samples/dmAutoPM_full.RData")
...
# Save
save(samples, file = "samples/sAutoPM_full.RData")
...
# 
# # Save
# save(hsamples, file = "samples/hsAutoPM_full.RData")