...
map_B <- empty.map(
  ...
  levels = c("C", "N", "P", "FAKEB"))
map_B
length(map_B)
length(levels(map_B))

map_B[1:96] <- c("C",  "C",  "FAKEB","FAKEB",
                 "C",  "C",  "FAKEB","FAKEB",
                 "C",  "C",  "FAKEB","FAKEB",
...
p.prior <- prior.p.dmc(
  ...
  p1 = p.vector,                           
  p2 = c(0.2, 0.1, rep(1, 3), rep(1, 40), rep(1, 3)), 
  lower = c(0.1, 0, rep(0, 3), rep(-Inf, 40), rep(0, 3)),
  upper = c(1, 10, rep(Inf, 3), rep(Inf, 40), rep(Inf, 3))
)
length(p.prior)
length(p.vector)
...
save(dm, file = "samples/dmAutoPM_v_sdvS.RData")
...
# Save
save(samples, file = "samples/sAutoPM_v_sdvS.RData")

# Load
print(load("samples/sAutoPM_v_sdvS.RData"))
...
# 
# # Save
# save(hsamples, file = "samples/hsAutoPM_v_sdvS.RData")
